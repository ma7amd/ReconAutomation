...
then
  ...
  nmap -sS -p- $target > recon/Ports.nmap
  sleep 1

  # fine tuning the ports extracted
  cat Ports.nmap | grep "open" | grep -v "filtered" | cut -d "/" -f 1 | sort -u | sort | xargs | tr ' ' ',' > OnlyPort.txt

  echo "Starting Just in Details Port Scanning"
  nmap -sC -sV -A -p $(cat OnlyPort.txt) $target > recon/initial_nmap.txt
  sleep 1
